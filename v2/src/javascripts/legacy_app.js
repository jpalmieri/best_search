...
const App = (function() {
  ...
  return {
    ...
    handleChangedQuery: function(event) {
      this.toggleSearchBtn(); // Show 'x' to clear input

      if (this.$(event.target).val()) {
        this.$(event.target)
          .siblings(".query-clear")
          .show();
      } else {
        this.$(event.target)
          .siblings(".query-clear")
    ...
    toggleSearchBtn: function() {
      var atLeastOneQueryHasText = _.some(
        this.$(".query"),
        function(queryInput) {
    ...
    startSearch: function() {
      ...
      this.ajax("requestItems", this.buildApiUrl()); // Avoid actual form submission, which redirects to new page
    ...
    buildApiUrl: function() {
      ...
      var apiUrl = API_PATH + ENDPOINT_PATH[searchType]; // Include locale in resource url if searcing articles
    ...
    processResults: function(data) {
      var itemType = this.$(".search-types a")
        .closest("li.active")
        .data("item");
      var results = data[itemType]; // Pass results through each selected filter

      _.each(
        this.$(".query"),
        function(query) {
          if (this.$(query).val()) {
            var filterType = this.$(query)
              .closest("tr")
              .data("filter");
      ...
      this.displayResults(results, itemType); // Get additional pages of api request results
    ...
    filterBy: {
      ...
      note: function(triggers) {
        ...
        var results = _.filter(
          ...
          function(trigger) {
            ...
            var notifications = _.filter(
              this.getNotifications(trigger),
      ...
      content: function(dcItems) {
        ...
        var results = _.filter(
          ...
          function(dcItem) {
            ...
            var contents = _.filter(this.getContents(dcItem), function(
              content
            ) {
      ...
      getTagValues: function(item) {
        var tagActions = _.filter(item.actions, function(action) {
          return (
            action.field.indexOf("_tags") > -1 ||
            action.field.indexOf("custom_fields_") > -1
          );
        });

        var values = _.pluck(tagActions, "value"); // Remove null values
      ...
      getComments: function(macro) {
        var actions = _.filter(macro.actions, function(action) {
          return (
            action.value &&
            ["comment_value", "comment_value_html"].includes(action.field)
          );
        });
